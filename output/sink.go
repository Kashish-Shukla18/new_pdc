...
// Redis writes are pipelined. Postgres writes are buffered and flushed off the ingest path.
type Sink struct {
	redis         *redis.Client
...
func NewSinkFromEnv(ctx context.Context) (*Sink, error) {
	...
	log.Printf("sink ready: redis=%s postgres history on", redisAddr)
	return &Sink{
		redis:         redisClient,
		history:       history,
		redisTTL:      redisTTL,
...
// Store writes one reading to Redis, then enqueues it for Postgres (non-blocking).
